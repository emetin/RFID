...
- `POST /v1/admin/encoding/batches` creates a durable batch and atomically
  allocates unique `GTX96` EPC jobs. It requires `chain_admin`.
- `GET /v1/admin/encoding/batches` lists progress; adding a batch ID returns one batch.
- `GET /v1/customer/encoding/batches` requires `encoding:read`.
- `POST /v1/customer/encoding/jobs/claim` requires `encoding:work`, `stationId`
  and an idempotency key. It returns one leased job or `job: null`.
- `POST /v1/customer/encoding/jobs/finish` requires `encoding:work`, the station,
  lease token and readback result. Matching readback verifies and registers the
  asset; a failure creates a replacement job with a new EPC.

Multiple writer stations coordinate through database leases, not local counters.

## Customer integration API

ERP, WMS and inventory connectors use a tenant-scoped Bearer credential from
`CUSTOMER_API_CREDENTIALS`. Credentials have an explicit `clientId`, one
`tenantId` and only the required scopes. The API never accepts a caller-supplied
tenant override.

```http
Authorization: Bearer <customer-service-token>
Idempotency-Key: catalog-sync-2026-07-21-001
Content-Type: application/json
```

`Idempotency-Key` is mandatory for mutations. Repeating the same method, path,
client and key returns the original result with `Idempotency-Replayed: true`.
Each credential is limited to 120 requests per minute per API process.

### `GET /v1/customer/catalog`

Requires `catalog:read`. Returns the customer's product master.

### `POST /v1/customer/catalog`

Requires `catalog:write`. Accepts the same product JSON or CSV contract as the
admin catalog import.

### `POST /v1/customer/assets`

Requires `assets:write`. Accepts JSON or CSV EPC-to-SKU mappings.

### `GET /v1/customer/inventory`

Requires `inventory:read`. Returns RFID physical, registered, available and
status totals with the SKU-level inventory lines.

Customer API mutations are written to the tenant audit history. Production
tokens must be randomly generated, stored in a secret manager and rotated.
...
### `GET /v1/admin/zoho/catalog`

Returns the last read-only Zoho mapping report generated by
`npm run zoho:mapping-report`. It exposes eligible item IDs, SKUs and names to
the authenticated dashboard without exposing OAuth credentials to the browser.

### `POST /v1/admin/zoho/assets/register`

Registers one scanned hexadecimal EPC against an eligible Zoho SKU. The request
also supplies `unitsPerBox`, `boxesPerPallet` and optional `category`; the server
upserts the RFID packaging record before creating the asset. The EPC contributes
to physical inventory only after its first accepted reader observation.
